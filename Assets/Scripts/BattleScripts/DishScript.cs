...
public class DishScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        else if(dish == "Bowl")
        {
            ...
            rb.velocity = new Vector2((direction.normalized.x*speed) + (Mathf.Cos(time*Time.fixedDeltaTime*Mathf.Deg2Rad*5) * 1), (direction.normalized.y*speed) + (Mathf.Sin(time*Time.fixedDeltaTime*Mathf.Deg2Rad*5) * 1));
        }
    }
